<div class="drawer-side z-50 ring-base-300 shadow">
    ...
    <ul class="menu p-4 w-80 min-h-full bg-base-200 text-base-content">
        ...
        @if (
            (auth()->user() && auth()->user()->hasPermission('management_users')) ||
                auth()->user()->hasPermission('management_roles'))
            {{-- User Management --}}
            <li>
                <details open="">
                    <summary class="group">
                    ...
                    <ul>
                        {{-- User --}}
                        @if (auth()->user() && auth()->user()->hasPermission('management_users'))
                        ...
                        @endif
                        {{-- Role --}}
                        @if (auth()->user() && auth()->user()->hasPermission('management_roles'))
                            <li>
        ...
        @if (
            (auth()->user() && auth()->user()->hasPermission('management_jenis')) ||
                auth()->user()->hasPermission('management_department') ||
            ...
            <li>
                <details open="">
                    ...
                    <ul>
                        @if (auth()->user() && auth()->user()->hasPermission('management_jenis'))
                        ...
                        @endif
                        @if (auth()->user() && auth()->user()->hasPermission('management_departement'))
                            <li>
                        ...
                        @if (auth()->user() && auth()->user()->hasPermission('management_product'))
                            ...
                            <li><a href="{{ route('unit.index') }}"
                                    class="{{ request()->routeIs('unit.index') ? 'active bg-base-300 font-semibold' : '' }}">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="currentColor" class="size-4">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            d="m21 7.5-9-5.25L3 7.5m18 0-9 5.25m9-5.25v9l-9 5.25M3 7.5l9 5.25M3 7.5v9l9 5.25m0-9v9" />
        ...
        @endif
        @if (
            (auth()->user() && auth()->user()->hasPermission('management_service')) ||
                auth()->user()->hasPermission('download_report'))
            {{-- Manajement Service --}}
            <li>
                <details open="">
                    <summary class="group">
                    ...
                    <ul>
                        @if (auth()->user() && auth()->user()->hasPermission('management_service'))
                            @if (auth()->user()->role->name == 'Admin' || auth()->user()->role->name == 'PIC')
        ...
        @endif
        <li>
            <a href="mailto:aditya@heyadit.my.id" class="btn btn-success mt-6">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-5">
                    <path
